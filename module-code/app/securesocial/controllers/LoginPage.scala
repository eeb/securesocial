...
class LoginPage @Inject() (
  ...
  implicit val cSRFAddToken: CSRFAddToken) extends BaseLoginPage
...
trait BaseLoginPage extends SecureSocial {
    ...
    UserAwareAction { implicit request =>
      if (request.user.isDefined) {
        ...
        val to = (if (enableRefererAsOriginalUrl) {
          ...
          None
        }).getOrElse(config.landingUrl)
        logger.debug("User already logged in, skipping login page. Redirecting to %s".format(to))
        Redirect(to)
      } else {
        if (enableRefererAsOriginalUrl) {
          SecureSocial.withRefererAsOriginalUrl(Ok(env.viewTemplates.getLoginPage(config.loginForm)))
        } else {
          Ok(env.viewTemplates.getLoginPage(config.loginForm))
        }
      }
    }
  }
  ...
  def logout = UserAwareAction.async {
    implicit request =>
      val redirectTo = Redirect(config.configObj.getString(onLogoutGoTo).getOrElse(env.routes.loginPageUrl))
      val result = for {
        user <- request.user
